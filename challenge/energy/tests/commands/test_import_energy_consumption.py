...
class TestCommandImportEnergyConsumption:
    def test_happy_path(self, valid_csv_file):
        ...
        call_command("import_energy_consumption", valid_csv_file, stdout=out)

        assert 'Successfully imported "3" energy consumptions' in out.getvalue()
    ...
    def test_execute_two_times(self, valid_csv_file):
        ...
        call_command("import_energy_consumption", valid_csv_file, stdout=out1)
        call_command("import_energy_consumption", valid_csv_file, stdout=out2)
...
class TestFileValidation:
    def test_call_without_file_path_fails(
        self,
    ):
        out = StringIO()
        ...
        with pytest.raises(
            ...
            call_command("import_energy_consumption", stdout=out)

    def test_file_not_found_raises_error(self):
        out = StringIO()

        with pytest.raises(CommandError, match='File "/file/path" not found'):
            call_command("import_energy_consumption", "/file/path", stdout=out)
    ...
    def test_invalid_columns_raises_error(self, invalid_format_file):
        ...
        with pytest.raises(
            ...
            call_command("import_energy_consumption", invalid_format_file, stdout=out)

    def test_empty_file_raises_error(self, empty_file):
        out = StringIO()

        with pytest.raises(CommandError, match=f'File "{empty_file}" is empty'):
            call_command("import_energy_consumption", empty_file, stdout=out)
    ...
    def test_csv_with_invalid_dates_raises_error_and_rollback(
        ...
        with pytest.raises(
            ...
            call_command(
                "import_energy_consumption", invalid_dates_csv_file, stdout=out
            )
    ...
    def test_csv_invalid_source_raise_error_and_rollback(self, invalid_source_csv_file):
        ...
        with pytest.raises(
            ...
            call_command(
                "import_energy_consumption", invalid_source_csv_file, stdout=out
            )
